...
from Akshi import (COUNT_PM, CMD_HELP, BOTLOG, PM_AUTO_BAN,
                     LASTMSG, LOGGER, bot)
...
async def permitpm(event):
    ...
    if event.is_private and event.chat_id != 777000 and event.chat_id != self_user.id and not (
        ...
        try:
            from Akshi.sql.antipm_sql import is_approved
            from Akshi.sql.globals import gvarstatus
...
async def auto_accept(event):
    ...
    if event.is_private and event.chat_id != 777000 and event.chat_id != self_user.id and not (
        ...
        try:
            from Akshi.sql.antipm_sql import is_approved
            from Akshi.sql.antipm_sql import approve
            from Akshi.sql.globals import gvarstatus
        except AttributeError:
            return
...
async def notifoff(noff_event):
    ...
    try:
        from Akshi.sql.globals import addgvar
    except AttributeError:
...
async def notifon(non_event):
    ...
    try:
        from Akshi.sql.globals import delgvar
    except AttributeError:
...
async def disapprovepm(disapprvpm):
    try:
        from Akshi.sql.antipm_sql import dissprove
...
async def add_pmsg(cust_msg):
    ...
    try:
        import Akshi.sql.globals as sql
    except AttributeError:
